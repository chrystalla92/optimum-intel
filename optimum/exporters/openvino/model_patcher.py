...
def _olmoe_sparse_moe_block_forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
    batch_size, sequence_length, hidden_dim = hidden_states.shape
    hidden_states = hidden_states.view(-1, hidden_dim)
    # router_logits: (batch * sequence_length, n_experts)
    router_logits = self.gate(hidden_states)
    ...
    routing_weights = F.softmax(router_logits, dim=1, dtype=torch.float)
    routing_weights, selected_experts = torch.topk(routing_weights, self.top_k, dim=-1)
    # Conditional normalization: only normalize if norm_topk_prob is True
...
class OlmoeModelPatcher(OVDecoderModelPatcher):
    def __enter__(self):
        ...
        for layer in self._model.model.layers:
            layer.mlp._unpatched_forward = layer.mlp.forward
            layer.mlp.forward = types.MethodType(_olmoe_sparse_moe_block_forward, layer.mlp)

    def __exit__(self, exc_type, exc_value, traceback):
        super().__exit__(exc_type, exc_value, traceback)

        for layer in self._model.model.layers:
            layer.mlp.forward = layer.mlp._unpatched_forward
